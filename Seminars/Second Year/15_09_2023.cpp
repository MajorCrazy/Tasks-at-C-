...
public:
	...
	Node* successor()
	{
		...
		else
		{
			...
			current = parent; //for min
			if (current->data > data)
				return current;
			for (; current->parent != NULL && current->data < data; current = current->parent);
...
public:
	...
	TreeIterator& operator=(const TreeIterator& it) { ptr = it.ptr; return *this;  } //*this - указатель на итератор, а не просто указатель
