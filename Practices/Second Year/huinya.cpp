...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0) {
            return false; // Attempt to add document with negative id
        }
        if (documents_.count(document_id)) {
            return false; // Attempt to add document with id that already exists
        }
        if (!IsValidWord(document)) {
            return false; // Document contains special characters
        }
...
private:
    ...
    bool IsValidQuery(const string& query) const {
        // Check for more than one minus sign before words
        if (query.find("--") != string::npos) {
            return false;
        }
        // Check for minus sign without word after it
...
int main() {
    ...
    if (search_server.FindTopDocuments("пушистый ухоженный кот"s, actual_documents)) {
        for (const Document& document : actual_documents) {
            PrintDocument(document);
        }
    }

    cout << "BANNED:"s << endl;
    vector<Document> banned_documents;
    if (search_server.FindTopDocuments("пушистый ухоженный кот"s, DocumentStatus::BANNED, banned_documents)) {
    ...
    vector<Document> even_id_documents;
    if (search_server.FindTopDocuments("пушистый ухоженный кот"s,
                                       [](int document_id, DocumentStatus status, int rating) {
                                           return document_id % 2 == 0;
                                       },
                                       even_id_documents)) {
        for (const Document &document : even_id_documents) {
            PrintDocument(document);
        }
    }
    return 0;
