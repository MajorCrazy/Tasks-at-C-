...
{
  for (int i = 1; i < BaseMatrix<T>::height - 1; i++)
    {
        for (int j = 1; j < BaseMatrix<T>::width - 1; j++)
        {
            T currentValue = BaseMatrix<T>::ptr[i][j];
            if (currentValue < BaseMatrix<T>::ptr[i-1][j] && currentValue < BaseMatrix<T>::ptr[i+1][j] &&
                currentValue < BaseMatrix<T>::ptr[i][j-1] && currentValue < BaseMatrix<T>::ptr[i][j+1])
            {
                cout << "Local minimum found at position (" << i << ", " << j << ")\n";
            }
        }
    }
}
